...
const OP_COLORS: Record<OpType, string> = {
  ...
  UPDATE: "text-[#3b82f6] bg-[#3b82f6]/10 border-[#3b82f6]/20",
    ...
    <div>
      ...
      <div className="flex gap-3 mb-6">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-[#888888] border border-[rgba(255,255,255,0.08)] hover:text-[#ededed] transition-colors disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[#111111] overflow-hidden">
          <div className="px-5 py-4 border-b border-[rgba(255,255,255,0.06)] flex items-center justify-between">
            <div className="text-sm font-medium text-[#ededed]">DB Operation Events</div>
            {running && <span className="w-2 h-2 rounded-full bg-[#22c55e] animate-pulse" />}
          </div>
          <div className="divide-y divide-[rgba(255,255,255,0.04)] max-h-[380px] overflow-y-auto">
            {events.length === 0 ? (
              ...
              events.map((evt) => (
                <div key={evt.id} className="px-5 py-3 flex items-center gap-3 hover:bg-[rgba(255,255,255,0.02)] transition-colors">
                  <span className={`text-xs font-mono font-semibold px-1.5 py-0.5 rounded border flex-shrink-0 ${OP_COLORS[evt.op]}`}>
                    {evt.op}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono text-[#ededed]">{evt.table}</span>
                      <span className="text-xs text-[#888888]">row #{evt.row_id}</span>
                      <span className="text-xs text-[#888888]">·</span>
                      <span className="text-xs text-[#888888]">{evt.tenant}</span>
                    </div>
                  </div>
                  <span className="text-xs text-[#888888] font-mono flex-shrink-0">{evt.latencyMs}ms</span>
                </div>
              ))
            )}
          </div>
        ...
        <div className="rounded-xl border border-[rgba(255,255,255,0.08)] bg-[#111111] p-5">
          <p className="text-xs text-[#888888] uppercase tracking-widest font-medium mb-4">Consumer Projections</p>
          <div className="space-y-4">
            {consumers.map((c) => (
              <div key={c.id} className="rounded-lg border border-[rgba(255,255,255,0.06)] bg-[#0a0a0a] p-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-semibold text-[#ededed] font-mono">{c.name}</span>
                  <span className="text-xs text-[#3b82f6] font-mono">{c.processed.toLocaleString()}</span>
                </div>
                <div className="text-xs text-[#888888] mb-2">{c.label}</div>
